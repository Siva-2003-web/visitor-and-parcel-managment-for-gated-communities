...
export const createParcel = async (
  ...
  const [result] = await db.execute<ResultSetHeader>(
    `INSERT INTO visitors_parcels (
       resident_id, parcel_number, full_name, phone, purpose, status, record_type, received_at
     ) VALUES (:resident_id, :parcel_number, :full_name, :phone, :purpose, :status, 'parcel', NOW())`,
    {
...
export const getParcels = async (
  ...
  const [rows] = await db.query<ParcelRow[]>(
    `SELECT id, resident_id, full_name as sender_name, phone as sender_phone, purpose as description, status, received_at, acknowledged_at, collected_at, created_at, updated_at, parcel_number
